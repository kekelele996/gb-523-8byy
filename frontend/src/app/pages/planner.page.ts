...
@Component({
  ...
  imports: [DecimalPipe, ReactiveFormsModule, MatButtonModule, MatCheckboxModule, MatFormFieldModule, MatInputModule, MatSelectModule,
    ConstraintBadgeComponent, FreezeDriftPanelComponent, LucideAngularModule],
    ...
    <div class="page planner-page">
      ...
      <section class="control-bar">
        ...
        @if (selected(); as active) {
          ...
          @if (active.rebuilt_from_id) {<span class="rebuild-tag">rebuild of #{{ active.rebuilt_from_id }}</span>}
          ...
          @if (active.scenario_status === 'pending_review' && canPlan() && !active.is_superseded) {<button mat-stroked-button (click)="rebuild(active)" [disabled]="rebuilding()"><lucide-icon name="rotate-ccw" [size]="16" /> {{ rebuilding() ? 'Rebuilding...' : 'Rebuild on latest data' }}</button>}
          @if (active.scenario_status === 'pending_review' && canApprove()) {<button mat-flat-button color="primary" (click)="transition('approved')" [disabled]="active.approval_blocked" [title]="active.approval_block_reason"><lucide-icon name="check-circle-2" [size]="16" /> Approve</button>}
        }
      </section>

      @if (selected(); as active) {
        @if (active.scenario_status !== 'draft') {
          <app-freeze-drift-panel [scenario]="active" title="Frozen planning inputs" />
        }
...
export class PlannerPage {
  ...
  transition(target: ScenarioStatus): void { const scenario = this.selected(); if (!scenario) return; this.scenarioApi.transition(scenario.id, scenario.version, target, 'Reviewed in planning workbench').subscribe({ next: (result) => { this.store.select(result); this.scenarios.update((items) => items.map((item) => item.id === result.id ? result : item)); this.snack.open(`Scenario ${target.replace('_', ' ')}`, undefined, {duration: 2200}); }, error: () => this.load() }); }
  rebuild(scenario: LayoutScenario): void {
    this.rebuilding.set(true);
    this.scenarioApi.rebuild(scenario.id, scenario.version).pipe(finalize(() => this.rebuilding.set(false))).subscribe({ next: (result) => {
      this.scenarios.update((items) => [result, ...items.map((item) => item.id === result.id ? result : item)]);
      this.store.select(result);
      this.snack.open(`Rebuilt and re-evaluated: score ${result.score.toFixed(1)}`, undefined, {duration: 3000});
    }, error: () => this.load() });
  }
