...
@Component({
  ...
  imports: [DatePipe, DecimalPipe, MatButtonModule, MatFormFieldModule, MatSelectModule, CapacityMeterComponent,
    ConstraintBadgeComponent, FreezeSummaryComponent, VersionComparePanelComponent,
    LucideAngularModule],
      ...
      <section class="review-queue">
        ...
        <div class="queue-items">
          @for (scenario of pending(); track scenario.id) {
            <article [class.drifted]="hasDrift(scenario)">
              <div><span class="status pending_review">pending review</span><h2>{{ scenario.name }}</h2><p>v{{ scenario.version }} / {{ scenario.algorithm_version }} / {{ scenario.assignments.length }} placements</p>
                <p class="frozen-line"><lucide-icon name="snowflake" [size]="12" /> Frozen {{ scenario.frozen_at ? (scenario.frozen_at | date:'MMM d, yyyy HH:mm') : 'unknown' }}</p>
                @if (hasDrift(scenario); as drifted) {
                  <p class="drift-line"><lucide-icon name="shield-alert" [size]="13" /> {{ scenario.input_drift?.added_count ?? 0 }} added / {{ scenario.input_drift?.removed_count ?? 0 }} missing / {{ scenario.input_drift?.changed_count ?? 0 }} changed</p>
                }
              </div>
              <div class="queue-score"><small>Score</small><strong>{{ scenario.score | number:'1.0-1' }}</strong></div>
              @if (hasDrift(scenario)) {
                <app-constraint-badge severity="critical" label="Input drift blocks approval" />
              } @else {
              ...
              <div class="queue-actions">
                <button mat-flat-button color="primary" [disabled]="approvalBlocked(scenario)" (click)="transition(scenario, 'approved')"><lucide-icon name="check-circle-2" [size]="16" /> Approve</button>
                @if (auth.hasRole('planner', 'admin')) {<button mat-stroked-button [disabled]="rebuildingId() === scenario.id" (click)="rebuild(scenario)"><lucide-icon name="refresh-cw" [size]="15" /> {{ rebuildingId() === scenario.id ? 'Rebuilding...' : 'Rebuild' }}</button>}
              </div>
              @if (hasDrift(scenario)) {<p class="blocking-line">{{ scenario.input_drift?.blocking_reason }}</p>}
            </article>
          } @empty {<div class="queue-empty">No scenarios are waiting for review.</div>}
        </div>
      </section>

      @if (driftScenario(); as drift) {
        <app-freeze-summary [scenario]="drift">
          <span rebuildAction>
            @if (auth.hasRole('planner', 'admin')) {<button mat-flat-button color="primary" [disabled]="rebuildingId() === drift.id" (click)="rebuild(drift)"><lucide-icon name="refresh-cw" [size]="15" /> {{ rebuildingId() === drift.id ? 'Rebuilding...' : 'Rebuild & re-evaluate' }}</button>}
          </span>
        </app-freeze-summary>
      }
  ...
  `,
  styles: [`
    .review-queue{margin-bottom:16px;background:#15191d;border:1px solid #050607;color:#eef1f3}.queue-heading{min-height:48px;display:flex;align-items:center;justify-content:space-between;padding:10px 14px;border-bottom:1px solid #3a4045}.queue-heading>span{display:flex;align-items:center;gap:8px}.queue-heading strong{font-size:13px}.queue-heading small{color:#aeb6bc;font-size:10px;text-transform:uppercase}.queue-items{display:grid;grid-template-columns:repeat(auto-fit,minmax(320px,1fr))}.queue-items article{min-height:128px;display:grid;grid-template-columns:minmax(0,1fr) 60px;gap:9px 14px;align-items:center;padding:14px;border-right:1px solid #3a4045}.queue-items article:last-child{border-right:0}.queue-items article.drifted{background:#241817}.queue-items h2{margin:8px 0 0;font-size:14px}.queue-items p{margin:4px 0 0;color:#aeb6bc;font-size:9px}.queue-items .frozen-line{display:flex;align-items:center;gap:4px;color:#b8c0c5}.queue-items .drift-line{display:flex;align-items:center;gap:4px;color:#e6a09a;font-weight:700}.queue-items .blocking-line{grid-column:1/-1;color:#e6a09a;font-size:9px;line-height:1.4}.queue-score{text-align:right}.queue-score small,.queue-score strong{display:block}.queue-score small{color:#aeb6bc;font-size:9px;text-transform:uppercase}.queue-score strong{margin-top:3px;font-size:20px}.queue-items .queue-actions{grid-column:1/-1;display:flex;gap:8px;flex-wrap:wrap}.queue-items .queue-actions button{margin:0}.queue-empty{grid-column:1/-1;padding:30px;color:#aeb6bc;font-size:11px;text-align:center}.audit-grid{display:grid;grid-template-columns:minmax(0,1.3fr) minmax(330px,.7fr);gap:16px;margin-bottom:16px}.panel-header h2{display:flex;align-items:center;gap:7px}.compare-controls{display:grid;grid-template-columns:1fr 1fr;gap:4px 12px}.compare-controls .full{grid-column:1/-1}.boundary-list{padding:4px 14px}.boundary-list>div{display:grid;grid-template-columns:minmax(120px,1fr) minmax(105px,.8fr) 90px;align-items:center;gap:12px;padding:12px 0;border-bottom:1px solid #e5e8ea}.boundary-list>div:last-child{border-bottom:0}.boundary-list strong,.boundary-list small{display:block;letter-spacing:0}.boundary-list strong{font-size:11px}.boundary-list small{margin-top:3px;color:#68717a;font-size:9px}.events-panel .panel-header mat-form-field{width:190px}.summary-cell{max-width:240px;white-space:normal}.events-panel code{font-size:9px;color:#4f5860}.archive-row{display:flex;align-items:center;gap:10px;flex-wrap:wrap;margin-top:16px;padding:14px 0;border-top:1px solid #aeb6bd}.archive-row>span{margin-right:auto}.archive-row strong,.archive-row small{display:block}.archive-row strong{font-size:12px}.archive-row small{margin-top:3px;color:#68717a;font-size:10px}
...
export class AuditPage {
  ...
  readonly driftScenario = computed(() => this.pending().find((item) => item.input_drift?.has_drift) ?? null);

  hasDrift(scenario: LayoutScenario): boolean { return scenario.input_drift?.has_drift ?? false; }
  approvalBlocked(scenario: LayoutScenario): boolean { return scenario.has_critical_violation || this.hasDrift(scenario); }
  ...
  rebuild(scenario: LayoutScenario): void {
    ...
    this.scenarioApi.rebuild(scenario.id, scenario.version).subscribe({
      next: (rebuilt) => {
        this.rebuildingId.set(null);
        this.snack.open(`Rebuilt as #${rebuilt.id}; old scenario archived and remains queryable`, undefined, {duration: 3200});
        this.load();
      },
      error: () => { this.rebuildingId.set(null); this.snack.open('Rebuild rejected; the original scenario was not changed', undefined, {duration: 3200}); },
    });
  }
}
